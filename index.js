...
let atualizandoSchema = {}

monthSchema = new mongoose.Schema()
...
app.post('/stateMonth', express.json(), async (req, res) => {
    ...
    Object.keys(req.body).map((item, index) => {
        ...
        if (item === 'Name') {
            ...
            atualizandoSchema[item] = 'Number'
        }
    })
   
    monthSchema.add(atualizandoSchema);
...
// app.use('/', express.static(path.join(__dirname, 'client/build')))
// app.use('/Contas', takeBills, express.static(path.join(__dirname, 'client/build')))
// app.use('/Periodo', express.static(path.join(__dirname, 'client/build')))
// app.use('/Sobre', express.static(path.join(__dirname, 'client/build')))
